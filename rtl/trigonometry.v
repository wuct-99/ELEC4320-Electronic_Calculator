...
output  [31:0]  trig_sin_res;
output  [31:0]  trig_cos_res;
output          trig_sin_sign;
output          trig_cos_sign;
output          trig_done;
...
// Internal Signals
wire    [3:0]   cycle_cnt_nc;
wire    [3:0]   cycle_cnt;
wire    [15:0]  src1_unsigned;
wire            src1_is_0;
wire            src1_is_90;
...
// Control Logic
assign  cycle_cnt_nc    = cycle_cnt + 1'b1;
assign  trig_done       = (cycle_cnt == 4'd15)
dffre #(4) cycle_counter (.clk(clk), .rst_n(rst_n), .en(trig_vld), .d(cycle_cnt_nc), .q(cycle_cnt));
...
assign  src1_is_0       = (src1_unsigned == 16'b0);
assign  src1_is_90      = (src1_unsigned == 16'd90);

assign  trig_sin_res    = (src1_is_0)   ? 32'h0:
                          (src1_is_90)  ? 32'h0001_0000:
                          (y16[31])     ? (~y16 + 1'b1): y16;      
assign  trig_sin_sign   = y[16];

assign  trig_cos_res    = (src1_is_0)   ? 32'h0001_0000:
                          (src1_is_90)  ? 32'h0:
                          (y16[31])     ? (~x16 + 1'b1): x16;      
assign  trig_cos_sign   = x[16];
